...
// the scraped stage label (e.g. "55W Cl 11/12") must single out this row —
...
function koSlotTeams(r) {
  ...
  // …and the candidate must point at THIS row alone: a stage-less candidate
  // in a shared div/date/time slot could belong to either row.
  const slotRows = KNOCKOUT_AUG.filter((k) => k.div === r.div && k.d === r.d && k.t === r.t);
  ...
  for (const day of Object.keys(byDay).sort()) {
    ...
    for (const r of byDay[day]) {
      ...
      if (r.type === "event") {
        ...
        html += `<div class="match-row is-ko ${usa ? "is-usa" : ""}">${timeCell}<div class="m-div d-${r.div}">${DIVISIONS_AUG[r.div].short}</div><div class="m-label">${title}</div>${koRight}</div>`;
      } else {
