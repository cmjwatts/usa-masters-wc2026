  ...
  for (const day of Object.keys(byDay).sort()) {
    ...
    for (const r of byDay[day]) {
      ...
      if (r.type === "event") {
        ...
        html += `<div class="match-row ${usa ? "is-usa" : ""}">${timeCell}<div class="m-div d-${r.div}">${DIVISIONS_AUG[r.div].short}</div><div class="m-match">${name(r.h)}${score}${name(r.a)}</div><div class="m-pitch">${pitchLabel(r.p)}${watch}</div></div>`;
      }
    }
    html += `</div>`;
  }
