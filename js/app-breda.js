...
// In the schedule, USA squads read "US <division>" (e.g. US MO65) so
// multi-team views make clear which team is playing.
function teamNameIn(code, div) {
  if (code === "USA") return `${TEAMS_BRE.USA.flag} US ${DIVISIONS_BRE[div].short}`;
...
function matchesFiltered() {
  ...
  if (state.showKO) {
    ...
    // would otherwise be invisible — synthesize a row for it. (Same logic
    // as app-aug.js; seen live 2026-08-13 when Breda's Saturday block moved
    // from morning to afternoon.)
    const covered = new Set(KNOCKOUT_BRE.map((k) => `${k.div}|${k.d}|${k.t}`));
    const seen = new Set();
  ...
  for (const day of Object.keys(byDay).sort()) {
    ...
    for (const r of byDay[day]) {
      ...
      if (r.type === "event") {
        ...
        html += `<div class="match-row is-ko ${usa ? "is-usa" : ""}">${timeCell}<div class="m-div d-${r.div}">${DIVISIONS_BRE[r.div].short}</div><div class="m-label">${title}</div>${koRight}</div>`;
      } else {
